<?php
if(!class_exists("WoofPetWordPressThemeBase")) {
	class WoofPetWordPressThemeBase {
    	public $key = "3ABD5E6FC7F464AF";
    	private $product_id = "18";
    	private $product_base = "woof";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_woof_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=woof_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
