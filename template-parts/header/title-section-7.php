<?php $woof_options = get_option('woof'); ?> <!-- Search Page -->    
...
<?php } else { ?> 	
    ...
    <div class="jumbotron jumbotron-fluid overlay">
        <div class="jumbo-heading">
			...
			<?php if(!empty($woof_options['src-page-title'])): ?>
			<h1><?php echo esc_attr(($woof_options['src-page-title']));?> <em><?php printf( esc_attr__( ' "%s"', 'woof' ), '<strong>' . get_search_query() . '</strong>' ); ?></em></h1>
			<?php else:?>
			<h1><?php esc_attr_e('Search Results for : ','woof');?> <em><?php printf( esc_attr__( ' "%s"', 'woof' ), '<strong>' . get_search_query() . '</strong>' ); ?></em></h1>
			<?php endif;?>
            </div>
			<!-- /section-heading -->
			...
			<nav aria-label="breadcrumb">
			    <ol class="breadcrumb">
				   ...
				   <li class="breadcrumb-item active" aria-current="page"><?php printf( esc_attr__( ' "%s"', 'woof' ), '<strong>' . get_search_query() . '</strong>' ); ?></li>
				</ol>
			</nav>				
			<?php } ;?>  	
        </div>
