<?php $woof_options = get_option('woof'); ?> <!-- Archive Page -->    
...
<?php } else { ?> 	
    ...
    <div class="jumbotron jumbotron-fluid overlay">
        <div class="jumbo-heading">
			...
			<?php if(!empty($woof_options['arch-page-title'])): ?>
			<h1><?php echo esc_attr(($woof_options['arch-page-title']));?> <em><?php single_month_title(' '); ?></em></h1>
			<?php else:?>
			<h1><?php esc_attr_e('Archive : ','woof');?> <em><?php single_month_title(' '); ?></em></h1>
			<?php endif;?>
            </div>
			<!-- /section-heading -->
			...
			<nav aria-label="breadcrumb">
			    <ol class="breadcrumb">
				   ...
				   <li class="breadcrumb-item active" aria-current="page"><?php single_month_title(' '); ?></li>
				</ol>
			</nav>				
			<?php } ;?>  	
        </div>
